...
use yew::{html, use_context, use_effect};
...
#[fc(LoginInternals)]
pub fn login_internals() -> Html {
    let ctx_ = use_context::<Model>();
    if ctx_.clone().is_none() {
        return html! {
            <div>
                <h3>{"Loading..."}</h3>
            </div>
    ...
    html! {
      ...
      <div class="rounded-md shadow-sm -space-y-px">
        <div>
          ...
          <input id="email-address" value={form
            .get("email")
            .unwrap_or(&"".to_string().to_owned())
            .to_string()} name="email" type="email" autocomplete="email" required={true
        ...
        <div>
          ...
          <input id="password" value={form
            .get("password")
            .unwrap_or(&"".to_string().to_owned())
            .to_string()} name="password" type="password" autocomplete="current-password" required={true} class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Password" />
        </div>
      </div>
